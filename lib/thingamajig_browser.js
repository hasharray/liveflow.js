...
setTimeout(function next(pending) {
  ...
  for (var i = 0; i < scripts.length; i++) {
    ...
    if (pending[script.name]) {
      continue;
    }

    pending[script.name] = setTimeout(function(script) {
      if (script.hasAttribute('src')) {
        pending[script.name] = setTimeout(function callback(headers, script) {
          var request = new XMLHttpRequest();
          request.open('HEAD', script.src);
          request.onload = function() {
            ...
            if (modified) {
              ...
              request.onload = function() {
                revaluate(request.responseText, script.name, function(output) {
                  eval(output.code);
                });
        ...
        pending[script.name] = setTimeout(function callback(textContent, script) {
          if (textContent != script.textContent) {
            revaluate(script.textContent, script.name, function(output) {
              eval(output.code);
            });
          }

          pending[script.name] = setTimeout(callback, 250, script.textContent, script);
